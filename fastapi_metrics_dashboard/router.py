...
def get_metrics_router(store: MetricsStore, config: Config) -> APIRouter:
    ...
    @metrics_router.get(
        "/config-b887e852-bd12-41f2-b057-1bd31eb5443e"
    )  # include_in_schema=False
    async def get_config():
        return {}

    @metrics_router.get(f"{config.custom_path}/json", status_code=200)
    ...
    @metrics_router.get(f"{config.custom_path}/table_overview", status_code=200)
    ...
    @metrics_router.delete(f"{config.custom_path}/reset", status_code=204)
